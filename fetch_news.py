...
ARCHIVE_PATH = Path("docs/data/archive.json")  # 見た記事を貯める履歴（追記のみ・削除しない）
...
def load_archive() -> list[dict]:
    """既存のアーカイブを読み込む（無ければ空、壊れていても落とさず空扱い）。"""
...
def update_archive(data: dict[str, list[dict]], now: datetime) -> tuple[int, int]:
    """取得記事を履歴へ追記する。link で重複判定し、最新から消えた記事も残し続ける。

    戻り値: (今回の追加件数, 累計件数)
    """
    archive = load_archive()
    seen = {r.get("link") for r in archive}
    added = 0
    for instrument, items in data.items():
        for it in items:
            link = it["link"]
            ...
            archive.append(
                {
                    ...
                    "link": link,
                    "source": it["source"],
                    "instrument": instrument,  # 最初に見つけた銘柄で記録
                    "published": published.isoformat() if published else None,
                    "first_seen": now.isoformat(),  # 初めてアーカイブした時刻
                }
            )
            added += 1
    # 新しい順に保持（公開日時を優先、無ければ初回取得時刻）
    archive.sort(
        key=lambda r: r.get("published") or r.get("first_seen") or "", reverse=True
    )
